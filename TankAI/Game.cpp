...
void Game::gameLoop()
{
	...
	while (!done)
	{
		...
		//else if (action == 4)
		{
			...
			{
				bullet.draw(tank.getBarrelX() + barrelX, tank.getBarrelY() - barrelY, velocity*power, ((degree*pi) / -180), gravity, &target);//przekazanie wspó³rzêdnych koñca lufy jako pocz¹tek trajektorii lotu
			}	
		}
